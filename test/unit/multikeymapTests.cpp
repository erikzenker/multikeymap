...
struct MultiKeyMapOneKeyTests {
    ...
    MultiKeyMap<Key, Value> multiKeyMap;
};
...
BOOST_FIXTURE_TEST_CASE(should_get_and_set_value_with_one_key, MultiKeyMapOneKeyTests)
{
    multiKeyMap(1) = "1";
...
BOOST_FIXTURE_TEST_CASE(should_return_all_values_of_subtree, MultiKeyMapOneKeyTests)
{
    std::vector<Value> values;
    std::vector<hana::tuple<Key>> keys;
...
struct MultiKeyMapTwoKeyTests {
    ...
    MultiKeyMap<Value, Key1, Key2> multiKeyMap;
};

BOOST_FIXTURE_TEST_CASE(should_return_all_values_of_two_keys_map, MultiKeyMapTwoKeyTests)
{
    std::vector<Value> values;
...
BOOST_FIXTURE_TEST_CASE(should_return_all_values_of_subtree_of_two_keys_map, MultiKeyMapTwoKeyTests)
{
    std::vector<Value> values;
    std::vector<hana::tuple<Key1, Key2>> keys;
